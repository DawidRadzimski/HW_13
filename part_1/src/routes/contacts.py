...
from part_1.src.schemas import ContactIn, ContactOut, ContactUpdate, UserDb
...
@router.get("/", response_model=List[ContactOut], description='No more than 10 requests per minute',
...
@router.get("/{contact_id}", response_model=ContactOut, description='No more than 10 requests per minute',
            dependencies=[Depends(RateLimiter(times=10, seconds=60))])
...
@router.get("/search/{keyword}", response_model=List[ContactOut], description='No more than 10 requests per minute',
            dependencies=[Depends(RateLimiter(times=10, seconds=60))])
...
@router.post("/", response_model=ContactOut, description='No more than 10 requests per minute',
             dependencies=[Depends(RateLimiter(times=10, seconds=60))])
...
@router.put("/{contact_id}", response_model=ContactOut, description='No more than 10 requests per minute',
            dependencies=[Depends(RateLimiter(times=10, seconds=60))])
...
@router.delete("/{contact_id}", response_model=ContactOut, description='No more than 10 requests per minute',
               dependencies=[Depends(RateLimiter(times=10, seconds=60))])
...
@router.get("/birthday/", response_model=List[ContactOut], description='No more than 10 requests per minute',
            dependencies=[Depends(RateLimiter(times=10, seconds=60))])
async def get_contacts_birthday_for_next_week(skip: int = 0, limit: int = 100, current_user: User = Depends(auth_service.get_current_user), db: Session = Depends(get_db)):
    contacts = await repository_contacts.get_contacts_birthday_for_next_seven_days(skip, limit, current_user, db)
    if contacts is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="No person has a birthday in common week.")
    return contacts